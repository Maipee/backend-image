...
router.get("/product/list.html",rle.isAdmin, function (req, resp) {
...
///////////////////////route "/admin/product/list" ////////////////////////////////////////////////
router.get("/product/update/:id",rle.isAdmin,function (req, resp) {
...
//function avec middleware upload.any()

router.post("/product/update/:id",rle.isAdmin,upload.any() ,function (req, resp) {
    ...
    var infoUploadFile = req.files;
    var pictureName = 'images/'+infoUploadFile[0].filename;
    ...
    console.log('INFOS DE L\'IMAGE : '+JSON.stringify(infoUploadFile));
    console.log('NOM DE L\'IMAGE : '+pictureName);
    
    //UPDATE `products` SET `label` = ?, `description` = ?, `ref` = ?, `price` = ?, `picture` = ?, `idvat` = ?, `deleted` = '0', `stock` = ? WHERE `products`.`id` = ?
    //Alors on passe second query pour la modification d'un produit recuperer par son id
        sqlhelper.pool.query("UPDATE `products` SET `label` = ?, `description` = ?, `ref` = ?, `price` = ?, `picture` = ?, `idvat` = ?, `stock` = ? WHERE `id` = ?",[label,description,ref,price,pictureName,idvat,stock,id], function (err, rows, fields) {
            //function qui renvoie une erreur si il y a un probleme
            // on verifie dans la console le valeur de err pour voir si il y a une erreur
           // console.log("err :" + err);
            //si il y a une erreur donc si la valeur err est differente de null
            if (err != null) {
                ...
                //sinon il n'y a pas d'erreur, donc on ajoute la nouvelle valeur au message
                message = "La recuperation des données c'est bien passé ";
                //on log pour voir si le message passe
                console.log('message bien reussi avec le message : ' + message);                
                //on redirige vers la même route pour voir les modifications
...
///////////////////////route "/admin/product/list" ////////////////////////////////////////////////
router.get("/product/delette/:id",rle.isAdmin, function (req, resp) {
...
router.get("/product/add",rle.isAdmin, function (req, resp) {

    var label = "Veuillez inserer le nom";
    var description = "Veuillez inserer la description";
    var ref = "Veuillez inserer la reference";
    var price = "Veuillez inserer le prix";
    var picture = "Veuillez inserer l'image";
    var idvat = "Veuillez inserer la tva (de 1 à 5)";
    var stock = "Veuillez inserer le stock";
    var id = "";
...
///////////////////////route "/admin/product/add" ////////////////////////////////////////////////
router.post("/product/add",upload.any(), rle.isAdmin, function (req, resp) {
    ...
    console.log('image : '+picture);
    console.log('tva : '+idvat);
    console.log('quantite magasinrthj : '+stock);


    var sql ="INSERT INTO `products` ( `label`, `description`, `ref`, `price`, `picture`, `idvat`, `deleted`, `stock`, `stockEntrepot`, `idFournisseur`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    
    sqlhelper.pool.query(sql,[label,description,ref,price,pictureName,idvat,deleted,stock,stockEntrepot,idFournisseur], function (err, rows, fields) {
            //function qui renvoie une erreur si il y a un probleme
            // on verifie dans la console le valeur de err pour voir si il y a une erreur
            //console.log("err :" + err);
            //si il y a une erreur donc si la valeur err est differente de null
