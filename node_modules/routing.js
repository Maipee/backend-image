...
router.post("/register.html",function (req,resp){
	...
	var regPrenom = /^\D{2,32}$/;
	var regNom = /^\D{2,64}$/;
	var regTel = /^\d{9}$/;
	var regNumeroDom = /^\w{1,16}$/;
	var regRueDom = /^\w{2,128}$/;
	var regCPDom = /^\d{5}$/;
	var regVilleDom = /^\w{2,32}$/;
...
router.get("/index.html",function (req,resp){
	mysqlhelper.pool.query('SELECT IDPRODUCT, SUM(QUANTITY) AS SOMME FROM ORDERS GROUP BY IDPRODUCT ORDER BY SOMME DESC LIMIT 3 ',function(err,rows,fields){
		if(err != null){
			console.log('*** Query SQL *** SELECT IDPRODUCT, SUM(QUANTITY) AS SOMME FROM ORDERS GROUP BY IDPRODUCT ORDER BY SOMME DESC LIMIT 3 - Error *** '+err);												
		}else
			console.log('*** Query SQL *** SELECT IDPRODUCT, SUM(QUANTITY) AS SOMME FROM ORDERS GROUP BY IDPRODUCT ORDER BY SOMME DESC LIMIT 3 - Success *** ');												
			if(typeof req.user !== 'undefined')
			{
				if(req.session.caddy){
					nbitem=req.session.caddy.length;
				...
				resp.render("/index.html.twig",{"list_new_arrivals":rows,"username":req.user.username,"nbitem":nbitem,"role":req.user.role});
			}
			else
			{
				if(req.session.caddy){
				...
				resp.render("/index.html.twig",{"list_new_arrivals":rows,"nbitem":nbitem});
			}
	});
});
