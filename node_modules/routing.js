...
router.get("/",function(req,resp){
	var newarrivals;
	mysqlhelper.pool.query('SELECT * FROM products ORDER BY id DESC LIMIT 5',function(err,rows,fields){
		newarrivals=rows;
		...
		{
			console.log('newarrivals OK');
			mysqlhelper.pool.query('SELECT * FROM products WHERE rand()>0.1 ORDER BY rand() LIMIT 3',function(err,rows,fields){
				...
				if(typeof req.user !== 'undefined')
				{
					...
					resp.render("index.html.twig",{"username":req.user.username,"nbitem":nbitem,"role":req.user.role,"newarrivals":newarrivals,"banners":banners});
				}
				else
				{
					if(req.session.caddy){
					...
					resp.render("index.html.twig",{"nbitem":nbitem,"newarrivals":newarrivals,"banners":banners});
				}
			});
		}
	});
