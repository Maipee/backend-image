...
router.get("/caddie/ajouter/:id/:quantite",function(req,resp){
	...
	if(typeof req.user !== 'undefined')
	{
		...
		connection.query('INSERT INTO commandes (product_ID, quantite, client_ID, prix, status_date, numero_commande, statut_en_cours) VALUES ("'+id+'", "'+quantite+'", (SELECT clients.ID FROM clients, user WHERE user.username = "'+username+'" AND user.email = clients.email), (SELECT priceHt FROM product WHERE ID = "'+id+'") * "'+quantite+'", NOW(), 555555, "dans le caddie") ',function(err,rows,fields){
			if(err != null)
			{
				console.log(err);
			}
